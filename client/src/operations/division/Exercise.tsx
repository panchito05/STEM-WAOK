...
export default function Exercise({ settings, onOpenSettings }: ExerciseProps) {
  ...
  const generateProblems = () => {
    ...
    for (let i = 0; i < settings.problemCount; i++) {
      newProblems.push(generateDivisionProblem(settings.difficulty));
    }
  ...
  const handleKeyboardInput = (value: string) => {
    if (value === "backspace") {
      ...
      // If there's not already a remainder, add it
      if (!userAnswer.includes("r")) {
        setUserAnswer(prev => prev + "r");
      }
    } else {
  ...
  const checkCurrentAnswer = () => {
    ...
    if (userAnswer.includes("r")) {
      const parts = userAnswer.split("r");
      quotient = parseInt(parts[0]) || 0;
      remainder = parseInt(parts[1]) || 0;
    } else {
      quotient = parseInt(userAnswer) || 0;
      remainder = 0;
    }
  ...
  const moveToPreviousProblem = () => {
    if (currentProblemIndex > 0) {
      ...
      if (previousAnswer) {
        const prevAnswer = previousAnswer.userRemainder > 0 
          ? `${previousAnswer.userQuotient}r${previousAnswer.userRemainder}` 
          : `${previousAnswer.userQuotient}`;
      ...
      <div className="p-6 bg-gray-50 rounded-lg mb-6">
        <div className="text-center">
          <div className={`text-3xl font-bold mb-6 flex justify-center items-baseline ${feedbackMessage ? (feedbackColor === "green" ? "text-green-600" : "text-red-600") : ""}`}>
            <span className="text-right w-16">{currentProblem.dividend}</span>
            <span className="mx-4">÷</span>
            <span className="text-right w-16">{currentProblem.divisor}</span>
            <span className="mx-4">=</span>
            <div className="border-b-2 border-gray-400 w-16 relative">
              <Input
                ...
                placeholder="Quotient[r]Remainder"
              />
            </div>
          </div>
          {feedbackMessage && (
          ...
          <p className="text-sm text-gray-500 mt-2">
            Format: "quotientremainder" (e.g., 5r2 for 5 with remainder 2)
          </p>
        </div>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3 sm:gap-4">
          ...
          <button
            className="w-12 h-12 bg-white rounded-lg shadow-sm border border-gray-300 text-base font-medium hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary"
            onClick={() => handleKeyboardInput("remainder")}
          >
            r
          </button>
          <button
