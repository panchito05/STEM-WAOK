...
import { validateAssociativePropertyProblem, validateProblem, validateUserAnswer, ValidationResult } from './validators';
...
export class Associative PropertyObserver {
  private static instance: Associative PropertyObserver;
  private listeners: Map<string, Function[]> = new Map();
  
  private constructor() {}
  
  public static getInstance(): Associative PropertyObserver {
    if (!Associative PropertyObserver.instance) {
      Associative PropertyObserver.instance = new Associative PropertyObserver();
    }
    return Associative PropertyObserver.instance;
  }
 ...
 */
export class Associative PropertyMiddleware {
  private static instance: Associative PropertyMiddleware;
  private observer: Associative PropertyObserver;
  
  // Cadenas de middleware para cada tipo de operación
  private problemMiddlewares: MiddlewareFunction<AssociativePropertyProblem>[] = [];
  private userAnswerMiddlewares: MiddlewareFunction<UserAnswer>[] = [];
  
  private constructor() {
    this.observer = Associative PropertyObserver.getInstance();
  ...
  public static getInstance(): Associative PropertyMiddleware {
    if (!Associative PropertyMiddleware.instance) {
      Associative PropertyMiddleware.instance = new Associative PropertyMiddleware();
    }
    return Associative PropertyMiddleware.instance;
  }
  
  /**
   * Agrega un middleware a la cadena de procesamiento de problemas
   */
  public addProblemMiddleware(middleware: MiddlewareFunction<AssociativePropertyProblem>): void {
   ...
   */
  public processProblem(problem: AssociativePropertyProblem): AssociativePropertyProblem {
   ...
   */
  private validateProblemMiddleware: MiddlewareFunction<AssociativePropertyProblem> = (problem) => {
    const validation = validateAssociativePropertyProblem(problem);
   ...
   */
  private logProblemMiddleware: MiddlewareFunction<AssociativePropertyProblem> = (problem) => {
    try {
 ...
 */
export function processProblem(problem: AssociativePropertyProblem): AssociativePropertyProblem {
  return Associative PropertyMiddleware.getInstance().processProblem(problem);
}
...
export function processUserAnswer(answer: UserAnswer): UserAnswer {
  return Associative PropertyMiddleware.getInstance().processUserAnswer(answer);
}
...
export function on(event: string, callback: Function): void {
  Associative PropertyObserver.getInstance().subscribe(event, callback);
}
...
export function off(event: string, callback: Function): void {
  Associative PropertyObserver.getInstance().unsubscribe(event, callback);
}

/**
 * Middleware auxiliar para aplicar reglas de negocio a problemas según su dificultad
 */
export function applyDifficultyRules(problem: AssociativePropertyProblem, difficulty: DifficultyLevel): AssociativePropertyProblem {
