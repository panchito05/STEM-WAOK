...
export default function Settings({ settings, onBack }: SettingsProps) {
  ...
  const headerTitle = isEnglish ? "🟣 Configuration - Associative Property" : "🟣 Configuración - Propiedad Asociativa";
        ...
        <div className={`p-4 rounded-lg shadow-sm ${theme.bgContainer} border ${theme.border}`}>
          ...
          <div className="mt-3 mb-2 space-y-1.5">
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Beginner:" : "Principiante:"}</span> {isEnglish ? "Visual learning with fruit emojis showing (a+b)+c = a+(b+c)" : "Aprendizaje visual con emojis de frutas mostrando (a+b)+c = a+(b+c)"}
            </p>
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Elementary:" : "Elemental:"}</span> {isEnglish ? "Numbers with colored groupings showing how regrouping gives same result" : "Números con agrupaciones de colores mostrando cómo reagrupar da el mismo resultado"}
            </p>
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Intermediate:" : "Intermedio:"}</span> {isEnglish ? "Interactive exercises filling blanks to complete associative property expressions" : "Ejercicios interactivos completando espacios en blanco para expresiones de propiedad asociativa"}
            </p>
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Advanced:" : "Avanzado:"}</span> {isEnglish ? "Verification exercises identifying correct associative property applications" : "Ejercicios de verificación identificando aplicaciones correctas de la propiedad asociativa"}
            </p>
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Expert:" : "Experto:"}</span> {isEnglish ? "Complex problems with multiple numbers and decimal operations" : "Problemas complejos con múltiples números y operaciones decimales"}
            </p>
          </div>
        </div>
