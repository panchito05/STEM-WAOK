        ...
        <div className="grid grid-cols-3 gap-1.5 sm:gap-2 max-w-xs mx-auto">
          {["1", "2", "3", "4", "5", "6", "7", "8", "9", "sequential_backspace", "0", "backspace"].map((key, idx) => (
            <Button
              ...
              variant={key === "sequential_backspace" ? "outline" : "outline"}
              className={`text-lg sm:text-xl h-11 sm:h-12 ${
                key === "sequential_backspace" ? "bg-red-50 text-red-600 hover:bg-red-100 active:bg-red-200" : 
                "bg-white hover:bg-gray-50 shadow-sm active:bg-gray-100"
              }`}
              onClick={() => {
                if (viewingPrevious || exerciseCompleted || waitingRef.current) return;
              ...
              disabled={waitingRef.current || exerciseCompleted || viewingPrevious || (!exerciseStarted && key !== "sequential_backspace" && key !== "backspace" && (key < '0' || key > '9'))}
            >
              {key === "backspace" ? <ChevronLeft className="h-5 w-5 sm:h-6 sm:w-6" /> : 
               key === "sequential_backspace" ? <span className="font-bold">&gt;</span> : key}
            </Button>
          ))}
        </div>
