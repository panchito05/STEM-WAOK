...
import eventBus, { on, off } from '@/lib/eventBus'; // Asumiendo que existe y es correcto
...
export default function Exercise({ settings, onOpenSettings }: ExerciseProps) {
  ...
  const moveToPreviousProblem = () => {
    ...
    if (!viewingPrevious) { // Si es la primera vez que se presiona "Previous" desde un problema activo
        setActualActiveProblemIndexBeforeViewingPrevious(currentProblemIndex); // Guardar el índice del problema activo
    }
    ...
    const prevIndexToView = viewingPrevious ? currentProblemIndex - 1 : currentProblemIndex -1 ;
    if(prevIndexToView < 0) return; // No ir más allá del primer problema
    ...
    if (prevAnswerEntry && prevProblemToView) {
        ...
        intPart = intPart.padStart(numIntBoxes, '0'); // Rellenar con ceros a la izquierda
        ...
        // Si no hay respuesta grabada para ese problema anterior
  ...
  const returnToActiveProblem = () => {
    ...
    // Restaurar el problema en el que el usuario estaba trabajando
    ...
    // El feedback y los digitAnswers para el problema activo se manejarán por el useEffect de [currentProblem, viewingPrevious].
    // Si el usuario dejó una respuesta parcial en el problema activo, se limpiará a menos que se implemente
    // un guardado más granular del estado de digitAnswers por problema.
    // Por ahora, limpiamos el feedback que pudo quedar de la vista previa.
    const activeProblemHistory = userAnswersHistory[actualActiveProblemIndexBeforeViewingPrevious];
    if(activeProblemHistory && !activeProblemHistory.isCorrect && activeProblemHistory.userAnswer !== undefined && !isNaN(activeProblemHistory.userAnswer)){
  ...
  const handleDigitBoxClick = (index: number) => {
    if (waitingForContinue || exerciseCompleted || viewingPrevious) return;
  ...
  const handleDigitInput = (value: string) => { // Para teclado en pantalla
    if (waitingForContinue || focusedDigitIndex === null || !currentProblem || exerciseCompleted || viewingPrevious) return;
    if (!exerciseStarted) startExercise();
    ...
    if (value === "backspace") {
      ...
      // No mover foco automáticamente al borrar con teclado en pantalla, el usuario puede querer borrar más.
    } else if (/[0-9]/.test(value)) {
      newAnswers[currentFocus] = value;
      // Mover foco después de ingresar un dígito
  ...
  useEffect(() => {
    const handlePhysicalKeyDown = (event: KeyboardEvent) => {
        ...
        const key = event.key;
        if (key >= '0' && key <= '9') {
            let newAnswers = [...digitAnswers]; // Obtener el estado más reciente
            newAnswers[focusedDigitIndex] = key;
            setDigitAnswers(newAnswers);
            // Mover foco después de la entrada
            if (inputDirection === 'rtl') {
                if (focusedDigitIndex > 0) setFocusedDigitIndex(focusedDigitIndex - 1);
            } else {
                if (currentProblem && focusedDigitIndex < currentProblem.answerMaxDigits - 1) setFocusedDigitIndex(focusedDigitIndex + 1);
            }
            ...
            setDigitAnswers(newAnswers);
            // No mover el foco al borrar con teclado físico, para permitir múltiples borrados. El usuario puede usar flechas.
            event.preventDefault();
        } else if (key === 'Enter') {
            checkCurrentAnswer();
            event.preventDefault();
            ...
            if (focusedDigitIndex > 0) setFocusedDigitIndex(focusedDigitIndex - 1);
            // Opcional: setInputDirection('ltr'); si se quiere que las flechas cambien la dirección de entrada futura
            event.preventDefault();
        } else if (key === 'ArrowRight') {
            if (currentProblem && focusedDigitIndex < currentProblem.answerMaxDigits - 1) {
                setFocusedDigitIndex(focusedDigitIndex + 1);
                // Opcional: setInputDirection('ltr');
            }
            event.preventDefault();
        }
    };
  ...
  const checkCurrentAnswer = () => {
    ...
    // Usar el índice del problema activo actual (que es currentProblemIndex en este punto)
    ...
    setActualActiveProblemIndexBeforeViewingPrevious(problemIndexForHistory); // Confirmar que este es el nuevo "activo"
    ...
    if (isCorrect) {
      ...
      if (settings.enableRewards) {
          ...
          if (Math.random() < getRewardProbability(rewardContext as any)) {
              awardReward('some_reward_id_correct' as any, { module: 'addition' }); // Ajusta el ID de la recompensa
  ...
  const handleContinue = () => {
    ...
    if (showLevelUpReward) { 
        setShowLevelUpReward(false); 
        setBlockAutoAdvance(false);
        // Después de subir de nivel, el usuario estaba en `actualActiveProblemIndexBeforeViewingPrevious`.
        // Regenerar el problema actual (que es el activo) con la nueva dificultad.
        setCurrentProblemIndex(actualActiveProblemIndexBeforeViewingPrevious); 
        const newProblemForLevelUp = generateAdditionProblem(adaptiveDifficulty);
        const updatedProblemsList = [...problemsList];
        updatedProblemsList[actualActiveProblemIndexBeforeViewingPrevious] = newProblemForLevelUp; // Reemplazar en la lista
        setProblemsList(updatedProblemsList);
        setCurrentProblem(newProblemForLevelUp);
        // El useEffect [currentProblem, viewingPrevious] se encargará de resetear cajones y foco.
        return; 
    }
    if (!blockAutoAdvance) {
        advanceToNextActiveProblem(); // Usar la función que avanza el índice activo
    }
  };
  ...
  if (!currentProblem && !exerciseCompleted) { 
     // Si la lista tiene problemas pero currentProblem es null (estado inesperado), intentar recuperar el activo.
  ...
  // Fallback final si currentProblem sigue siendo null después de los chequeos y no está completado.
  ...
  // El progreso se basa en cuántos problemas del set actual tienen una entrada en el historial (se han intentado)
  ...
  return (
    <div className="relative">
      ...
      <div className={`px-2 py-3 sm:px-4 sm:py-5 rounded-xl shadow-lg ${ adaptiveDifficulty === "beginner" ? "bg-sky-50 border-sky-200" : "bg-green-50 border-green-200" } border-2`}>
        ...
        <div className={`p-3 sm:p-4 rounded-lg mb-3 sm:mb-4 shadow-md bg-white min-h-[150px] sm:min-h-[180px] flex flex-col items-center justify-center`}>
          {currentProblem.layout === 'horizontal' ? (
            <div className="text-2xl sm:text-3xl font-bold flex items-center justify-center gap-1.5 sm:gap-2 flex-wrap">
              ...
              <span>{currentProblem.operands.length > 1 ? currentProblem.operands[1] : '?'}</span> {/* Manejar si solo hay un operando */}
              <span className="text-gray-600 mx-1">=</span>
            </div>
          ) : ( 
            <div className="inline-block text-right my-1 sm:my-2">
              ...
              <div 
                ...
                // Ajustar el ancho de la línea de suma para que sea un poco más ancha que los números
                style={{width: `${Math.max(5, sumLineTotalCharWidth + 2)}ch`, marginLeft: 'auto', marginRight: '0'}} // Usar 'ch' para ancho basado en caracteres
              />
            </div>
          )}
                ...
                <React.Fragment key={`digit-box-frag-${index}-${currentProblem.id}`}>
                  <div
                    ...
                    className={`${digitBoxBaseStyle} 
                                ${viewingPrevious ? digitBoxDisabledStyle : (focusedDigitIndex === index ? digitBoxFocusStyle : digitBoxBlurStyle)}
                                ${!viewingPrevious && !exerciseCompleted ? 'cursor-text hover:border-gray-400' : ''}`}
                    onClick={() => !viewingPrevious && !exerciseCompleted && handleDigitBoxClick(index)}
                  >
        ...
        <div className="grid grid-cols-3 gap-1.5 sm:gap-2 max-w-xs mx-auto">
          {["1", "2", "3", "4", "5", "6", "7", "8", "9", "", "0", "backspace"].map((key, idx) => (
            <Button
              ...
              onClick={() => !viewingPrevious && !exerciseCompleted && key && key !== "" && handleDigitInput(key)}
              disabled={waitingForContinue || exerciseCompleted || viewingPrevious || key === ""}
            >
          ...
          <TooltipProvider delayDuration={300}>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button 
                    variant="outline" size="sm" 
                    disabled={(!settings.showAnswerWithExplanation && !viewingPrevious) || viewingPrevious || exerciseCompleted} 
                    onClick={() => { 
                        if(currentProblem && !viewingPrevious && !exerciseCompleted) { 
                            if (singleProblemTimerRef.current) clearInterval(singleProblemTimerRef.current);
                            ...
                            if (!answerEntry || answerEntry.status !== 'revealed') {
                                 setUserAnswersHistory(prev => {
                                    const newHistory = [...prev];
