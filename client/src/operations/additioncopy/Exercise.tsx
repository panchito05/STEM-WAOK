...
export default function Exercise({ settings, onOpenSettings }: ExerciseProps) {
    ...
    if (isCorrect) {
      ...
      if (settings.maxAttempts > 0 && newAttempts >= settings.maxAttempts) {
        ...
        if (settings.enableCompensation) {
          ...
          const compensationProblem = generateSingleMultiplicationProblem(difficultyForCompensation);
  ...
  const handleContinue = useCallback(() => {
    ...
    if (showLevelUpReward) {
      setShowLevelUpReward(false);
      setBlockAutoAdvance(false);
      const newProblemForLevelUp = generateSingleMultiplicationProblem(adaptiveDifficulty);
      ...
      <div className={`px-2 py-3 sm:px-4 sm:py-5 rounded-xl shadow-lg min-h-[calc(100vh-8rem)] md:min-h-0 flex flex-col ${
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
            <div className="flex flex-row justify-between items-center sm:flex-col sm:items-start">
              <h2 className="text-lg sm:text-xl font-bold text-gray-800">{isEnglish ? "Multiplication" : "Multiplicación"}</h2>
            ...
            <div className="flex justify-center">
              <div className="inline-block text-right my-1 sm:my-2">
                ...
                <div
                  ...
                  style={{width: `${Math.max(5, productLineTotalCharWidth + 2)}ch`, marginLeft: 'auto', marginRight: '0'}}
                />
              </div>
            </div>
          )}
      ...
      {showProfessorMode && (
        <ProfessorMode
          ...
          onCorrectAnswer={(wasCorrect: boolean) => {
            ...
            const newProblem = generateSingleMultiplicationProblem(settings.difficulty);
