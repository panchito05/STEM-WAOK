...
export default function Settings({ settings, onBack }: SettingsProps) {
  ...
  const debouncedSave = useMemo(
    () =>
      debounce((settings: ModuleSettings) => {
        updateModuleSettings("subtraction", settings);
        console.log(`[SUBTRACTION] Guardando configuración (debounced):`, settings);
  ...
  const handleUpdateSetting = <K extends keyof ModuleSettings>(key: K, value: ModuleSettings[K]) => {
    ...
    if (key === "difficulty") {
      console.log("[SUBTRACTION] Guardando configuración de dificultad inmediatamente:", value);
      // Actualizamos directamente sin usar debounce para cambios de dificultad
      updateModuleSettings("subtraction", updatedSettings);
    } else {
  ...
  useEffect(() => {
    ...
    updateModuleSettings("subtraction", localSettings);
    console.log("[SUBTRACTION] Guardando configuración al cargar:", localSettings);
    ...
    return () => {
      if (!hasSavedRef.current) {
        ...
        updateModuleSettings("subtraction", localSettings);
        console.log("[SUBTRACTION] Guardando configuración al desmontar:", localSettings);
        ...
        try {
          ...
          if (currentSettings) {
            ...
            const updated = {
              ...parsed,
              subtraction: localSettings
            };
            localStorage.setItem(key, JSON.stringify(updated));
            console.log("[SUBTRACTION] Forzando actualización en localStorage:", updated);
          }
        } catch (e) {
      ...
      <div className="space-y-6">
        <div className={`p-4 rounded-lg shadow-sm ${theme.bgContainer} border ${theme.border}`}>
          ...
          <div className="mt-3 mb-2 space-y-1.5">
            ...
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Elementary:" : "Elemental:"}</span> {isEnglish ? "Two-digit number subtractions (25-13, 42-18)" : "Restas de números de dos dígitos (25-13, 42-18)"}
            </p>
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Intermediate:" : "Intermedio:"}</span> {isEnglish ? "Subtractions with large numbers (392-132, 500-216)" : "Restas con números grandes (392-132, 500-216)"}
            </p>
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Advanced:" : "Avanzado:"}</span> {isEnglish ? "4-digit number subtractions (5934-1247, 8742-3568)" : "Restas de números de 4 dígitos (5934-1247, 8742-3568)"}
            </p>
            <p className={`text-sm ${theme.accent} bg-white/60 rounded-md p-2 border ${theme.border}`}>
              <span className="font-bold">{isEnglish ? "Expert:" : "Experto:"}</span> {isEnglish ? "Very large number subtractions (70960-11650, 82730-39436)" : "Restas con números muy grandes (70960-11650, 82730-39436)"}
            </p>
          </div>
        </div>
